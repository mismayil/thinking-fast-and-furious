...
from transformers import BitsAndBytesConfig, Idefics2ForConditionalGeneration, VipLlavaForConditionalGeneration
import torch
...
PROCESSOR_MAP = {
    ...
    "vip-llava": "llava-hf/vip-llava-7b-hf"
}
...
def get_objects(text):
    # extract object references from the text (e.g., <object_id,image_id,coord_x,coord_y>)
    return list(set(re.findall(r'<[^>]+,[^>]+,[^>]+,[^>]+>', text)))

def parse_object_ref(object_ref):
    # parse object reference to extract object_id, image_id, and coordinates
...
def apply_perception_trick(predictions):
    # Apply post-hoc perception trick to the predictions
    # The trick is to extract the object references from the question text and replace the object IDs in the answer with the actual object references
...
def load_processor(model_path):
    ...
    if "idefics2" in model_path.lower():
        processor_path = PROCESSOR_MAP["idefics2"]
    elif "vip-llava" in model_path.lower():
        processor_path = PROCESSOR_MAP["vip-llava"]
    else:
        raise NotImplementedError(f"Processor not found for {model_path}")
...
def load_model(model_path, eval_mode=False, use_lora=False, use_qlora=False, device="cuda", cache_dir=CACHE_DIR):
    ...
    if "vip-llava" in model_path:
        model_class = VipLlavaForConditionalGeneration
